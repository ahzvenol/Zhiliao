...
class TopTabs : BaseHook() {
    ...
    private fun makeFakeTabInfo(): Any? {
        val fakeTabInfo = tabInfo?.constructors?.get(0)?.newInstance()
        val fakeTabStyle = tabStyle?.constructors?.get(0)?.newInstance()
        val fakeTextTab = textTab?.constructors?.get(0)?.newInstance()
